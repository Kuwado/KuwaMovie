...
const MediaItem = ({ media, type }) => {
    ...
    return (
        <>
            ...
            />
            <div className={`lt:flex mb:hidden absolute bottom-0 left-0 w-full h-full p-3 group-hover:opacity-100 opacity-0 transition-opacity ease-in items-end bg-gradient-to-t ${theme ? 'from-mainLight' : 'from-mainDark'} to-mainLight-[0]`}>
            ...
            <Link className='lt:flex mb:hidden absolute bottom-0 left-0 w-full h-full items-center justify-center group-hover:opacity-100 opacity-0 transition-opacity ease-in cursor-pointer'
                to={`/${type}/${media.id}`}>
                <div className={`rounded-[50%] p-2 bg-extra text-3xl ${theme ? 'hover:bg-textLight' : 'hover:bg-textDark'} hover:text-extra`}>
                    <FaCaretRight />
                </div>
            </Link>
        </>
